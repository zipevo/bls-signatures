...
func TestThreshold(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		skShares[i] = ThresholdPrivateKeyShare(sks, ids[i])
		pkShares[i] = ThresholdPublicKeyShare(pks, ids[i])
		sigShares[i] = ThresholdSignatureShare(sigs, ids[i])
	...
	recSk := ThresholdPrivateKeyRecover(skShares[:m-1], ids[:m-1])
	recPk := ThresholdPublicKeyRecover(pkShares[:m-1], ids[:m-1])
	recSig := ThresholdSignatureRecover(sigShares[:m-1], ids[:m-1])
	assert.False(t, recSk.EqualTo(sks[0]))
	assert.False(t, recPk.EqualTo(pks[0]))
	assert.False(t, recSig.EqualTo(sig))

	recSk = ThresholdPrivateKeyRecover(skShares[:m], ids[:m])
	recPk = ThresholdPublicKeyRecover(pkShares[:m], ids[:m])
	recSig = ThresholdSignatureRecover(sigShares[:m], ids[:m])
